...
const AddEmployees = ({addEmployees, setAddEmployees}) => {
  ...
  return (
    <div className={addEmployees ? `w-full h-screen modalBg fixed top-0 left-0 right-0 bottom-0 flex justify-center items-center z-[999] duration-300` : `w-full h-screen modalBg fixed top-0 left-0 right-[-200%] bottom-0 flex justify-center items-center z-[999] duration-300`}>
        <aside className={`absolute bg-white w-[450px] h-full flex flex-col justify-center items-start duration-300 ${addEmployees ? `right-0 top-0` : `right-[-200%] top-0`}`}>
